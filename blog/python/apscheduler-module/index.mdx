...
#### 删除 job
当个调度器删除 job 时， 该 job 也将从 job store 中删除。

你可以：
- 使用 remove_job() 并提供 job_id 和 store 的名称
- 通过 add_job() 的返回值 (apscheduler.job.Job) 实例的 remove() 方法

```python
job = sched.add_job(job_function, 'interval', minutes=2)
job.remvoe()

sched.add_job(job_function, 'interval', minutes=2, id="my_job_id")
...
```python
job = sched.add_job(job_function, 'interval', minutes=2, id='my_job_id')
job.pause()     # or
sched.pause_job('my_job_id')

job.resume()    # or
sched.resume_job('my_job_id')
```
...
也可以使用 add_job() 方法修改 job 信息，只需要指定参数 replace_existing=True 以及相同的 job_id 即可。
...
如果启动调度器的时候指定 paused=True，以这种方式启动的调度器直接就是暂停状态。
```python
